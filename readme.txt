Free Download Source Code "Electronics Parts Sales and Inventory System"

FIRST Download

1.XAMPP

2."TEXT EDITOR" NOTEPAD++ OR SUBLIME TEXT 3 / ETC.

3. "ComputerSalesInventorySystem"

4. Download the zip file/ download winrar

5. Extract the file and copy "ComputerSalesInventorySystem" folder

6.Paste inside root directory/ where you install xammp local disk C: drive D: drive E: paste: (for xampp/htdocs, 

7. Open PHPMyAdmin (http://localhost/phpmyadmin)

8. Create a database with name prince

6. Import prince.sql file(given inside the zip package in SQL file folder)

7.Run the script http://localhost/ComputerSalesInventorySystem

Computer Parts Sales and Inventory System

**LOGIN DETAILS** 

admin
user: admin
pass: admin

user
user: test
pass: test


****** https://www.campcodes.com ******
Subscribe to our Youtube Channel **** SerBermz ****